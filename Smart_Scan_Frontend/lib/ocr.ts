 ...
 */
async function extractTextWithOCRSpace(imagePath: string): Promise<string> {
  try {
    ...
    const apiKey = process.env.OCR_SPACE_API_KEY || 'helloworld'; // Free tier allows 'helloworld' as key
    ...
    // Extract text from all parsed text blocks
    let extractedText = '';
    if (result.ParsedResults && result.ParsedResults.length > 0) {
      extractedText = result.ParsedResults.map((block: any) => block.ParsedText).join('\n');
    }
 ...
 * Flow: 1. Try Gemini Vision API, 2. If fails, use OCR Space, 3. Parse with Gemini
 * @param {string} imagePath - Path to the image file
 * @returns {Promise<ContactInfo>} - Structured contact data
 */
export async function extractContactInfo(imagePath: string): Promise<ContactInfo> {
...
Return the JSON object now:`;
      ...
      if (!result) {
        // If all models failed (likely quota), don't throw - let it fall through to OCR Space
        console.log('All Gemini models failed. Will try OCR Space fallback.');
      } else {
        ...
        return {
          ...
          phones: Array.isArray(contactData.phones) ? contactData.phones.filter((p: any) => p && p.trim()) : [],
          ...
          rawText: contactData.rawText || text
        };
      }
    } catch (error) {
  ...
  try {
    ...
    // Return empty result if OCR Space also fails
    return {
      company: '',
      name: '',
      phones: [],
  ...
  // Use Gemini to parse the extracted text if available
...
Return the JSON object now:`;
  ...
  // Final fallback: If Gemini parsing failed or no Gemini API key, use basic regex parsing
